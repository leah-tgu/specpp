package org.processmining.estminer.candidatetraverser;
...
import org.processmining.estminer.plugins.PlugInStatistics;
...
public abstract class AbstractCandidateTraverser {
    ...
    protected ArrayList<MyPlace> roots;
...
//	protected String rootsPlacesNames; //for debugging
    ...
    public AbstractCandidateTraverser(final String[] transitions, final int[] outTrMapping, Parameters parameters) {
        ...
        this.limitDepth = parameters.isCut_depth();
    ...
    protected ArrayList<MyPlace> computeBaseRoots() {
        long startTime = System.currentTimeMillis();
        ArrayList<MyPlace> baseRoots = new ArrayList<MyPlace>();
...
//				if(!(transitions[in].equals(transitions[getTransitionIndex(0)]) && transitions[getTransitionIndex(out)].equals(transitions[0]))) {//exclude (start|end)
                baseRoots.add(new MyPlace(getMask(in), getMask(getMappedTransitionIndex(out))));
//				}
            }
        }
    ...
    protected String placeSetToString(ArrayList<MyPlace> placeSet) {
        String result = "";
        for (MyPlace place : placeSet) {
    ...
    protected boolean hasSingleMaximalOutTransition(MyPlace place) {
        if (place.getOutputTrKey() == getMask(largestOutIndex)) {
            return true;
        }
        return false;
    }
