...
import static org.processmining.estminer.specpp.evaluation.fitness.ReplayUtils.markingBasedReplay;
...
public class VMHComputations {
    ...
    public static Spliterator<IndexedItem<EnumSet<ReplayUtils.ReplayOutcomes>>> indexedFitnessComputation(VariantMarkingHistories vmh) {
        return vmh.getData()
                  .getIndexedVectors()
                  .map(ii -> new IndexedItem<>(toLocal(vmh, ii.getIndex()), markingBasedReplay(ii.getItem())))
                  .spliterator();
    }
