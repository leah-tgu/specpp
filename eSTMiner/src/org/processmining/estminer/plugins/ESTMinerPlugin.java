package org.processmining.estminer.plugins;
...
import org.processmining.estminer.candidatetraverser.BFSCandidateTraverser;
import org.processmining.estminer.candidatetraverser.DFSCandidateTraverser;
import org.processmining.estminer.discovery.AbstractDiscovery;
import org.processmining.estminer.discovery.DeltaDiscovery;
...
import org.processmining.estminer.implicitplaceremoval.OptimizationBasedImplicitPlaceRemover;
import org.processmining.estminer.implicitplaceremoval.ReplayBasedImplicitPlacesRemover;
import org.processmining.estminer.placecombinators.BFSDeltaCombinator;
...
@Plugin(
        ...
        returnLabels = {"Petri Net", "Initial Marking", "Final Marking", "Replayable Traces"},
        returnTypes = {Petrinet.class, Marking.class,
                Marking.class, XLog.class},
        userAccessible = true,
        help = "Runs the eSTMiner to discover garden models."
)
public class ESTMinerPlugin {

    @PluginVariant(requiredParameterLabels = {0})
    @UITopiaVariant(affiliation = "PADS", author = "Lisa Mannel", email = "mannel@pads.rwth-aachen.de")
    public Object[] discover(UIPluginContext context, XLog inputLog) throws InterruptedException {

        try {
            System.load("C:\\Users\\mannel\\.ProM\\packages\\lpsolve-5.5.4\\lib\\win64\\lpsolve55.dll");
        } catch (Exception e) {
        ...
        String logName = inputLog.getAttributes()
                                 .get("concept:name")
                                 .toString(); //specify logName (if not given by Cluster)
...
//TODO				discovery = new ClassicDiscovery(pM, candidates, evaluator, IPRemover, parameters, log);
        ...
        final int P_replayed = PlugInStatistics.instance().getNumFitting() + PlugInStatistics.instance()
                                                                                             .getNumUnfitting();
        ...
        System.out.println("Number of checked unfitting Places (Tree Traversal): " + PlugInStatistics.instance()
                                                                                                     .getNumUnfitting());
...
//	System.out.println("Of this for Replay (Treetraversal): " + PlugInStatistics.instance().getTimeReplay() + " ms");
        ...
        if (parameters.isRemoveImps()) {
            ...
            System.out.println("\n Number of places before final removing implicit places: " + discoveredPM.getPlaces()
                                                                                                           .size());
            ...
            discoveredPM = IPRemover.removeAllIPs(discoveredPM);
            System.out.println("Number of places after final removing implicit places: " + discoveredPM.getPlaces()
                                                                                                       .size() + "\n");
        ...
        double lengthOfShortestPathTroughModel = Double.parseDouble((String) alignedTraces.getInfo()
                                                                                          .get(Replayer.MAXMODELMOVECOST));
...
//	PlugInStatistics.instance().setAlignmentBasedFitness(-1); //TODO set -1 for cluster (cannot compute)
        ...
        if (printResults) {
            ...
            HashMap<Integer, HashMap<String, Integer>> levelstatistics = PlugInStatistics.instance()
                                                                                         .getLevelStatistics();
            String nl = "\n"; //new line indicator
    ...
    private boolean hasToFewLiveConnections(MyPlace place, boolean[] transitionsLiveness) {
        ...
        if (Integer.bitCount(clonedPlace.getInputTrKey()) < 1 || Integer.bitCount(clonedPlace.getOutputTrKey()) < 1) {
            return true;
        }
        return false;
    }
...
//			Progress progress = (Progress) context.getProgress(); //this does NOT work. it needs another progress
    ...
    private HashMap<Double, Integer> computeTraceFitnessSummary(PNRepResult alignedTraces, double lengthOfShortestPathTroughModel) {
        ...
        for (SyncReplayResult alignedTrace : alignedTraces) {
            ...
            double traceFitness = (double) 1 - ((double) costAlignedTrace / ((double) lengthAlignedTrace + (double) lengthOfShortestPathTroughModel));
