...
public class PrivatePlayground {
    ...
    public static void singleExecution() {
        SPECppOperations.configureAndExecute(new CustomSPECppConfigBundle(new MyParameters(.5, 5)), InputData.loadData(PrivatePaths.toAbsolutePath("temp/flatten_resources.xes"), PreProcessingParameters.getDefault())
                                                                                                             .getData(), false);
    }
    ...
    public static void weirdExecution() {
        SPECppOperations.configureAndExecute(new PlaceFocusedSPECppConfigBundle(), InputData.loadData(PrivatePaths.toAbsolutePath("temp/Customer.xes"), PreProcessingParameters.getDefault())
                                                                                            .getData(), false);
    }

    public static void multiParameterExecution() {
        DataSource<InputDataBundle> dataSource = InputData.loadData(PrivatePaths.toAbsolutePath(PrivatePaths.ROAD_TRAFFIC_FINE_MANAGEMENT_PROCESS), PreProcessingParameters.getDefault());
