...
public class ProMVisualizerAdapter {

    private final UIPluginContext context;
    private final boolean requiresCanceller;
    private final String visualizerName;
    private final String pluginName;
    private boolean cancelled;

    public ProMVisualizerAdapter(UIPluginContext context, String visualizerName, String pluginName, boolean requiresCanceller) {
        this.context = context;
        this.visualizerName = visualizerName;
        this.pluginName = pluginName;
        this.requiresCanceller = requiresCanceller;
    }
    ...
    public static List<ProMVisualizerAdapter> getAvailableVisualizers(UIPluginContext context, XLog inputLog) {
        ...
        Set<Pair<Integer, PluginParameterBinding>> logVisualizers = PluginManagerImpl.getInstance()
                                                                                     .find(Visualizer.class, JComponent.class, context.getPluginContextType(), true, false, false, inputLog.getClass());
        logVisualizers.addAll(PluginManagerImpl.getInstance()
                                               .find(Visualizer.class, JComponent.class, context.getPluginContextType(), true, false, false, inputLog.getClass(), ProMCanceller.class));
        ...
        for (Pair<Integer, PluginParameterBinding> pair : logVisualizers) {
            ...
            List<Class<?>> parameterTypes = plugin.getParameterTypes(pair.getSecond().getMethodIndex());
    ...
    public JComponent callPlugin(XLog log) {
        try {
            if (requiresCanceller) {
                return context.tryToFindOrConstructFirstNamedObject(JComponent.class, pluginName, null, null, log, (ProMCanceller) this::isCancelled);
            } else {
                return context.tryToFindOrConstructFirstNamedObject(JComponent.class, pluginName, null, null, log);
            }
        } catch (ConnectionCannotBeObtained connectionCannotBeObtained) {
            ProMTextArea proMTextArea = new ProMTextArea(false);
            proMTextArea.setText(connectionCannotBeObtained.getMessage());
            return proMTextArea;
        }
    }
